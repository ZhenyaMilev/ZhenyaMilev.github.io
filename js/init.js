...
function elisc_tm_modalbox_portfolio(){
	...
	button.on('click',function(){
			var element     = jQuery(this);
			var parent      = element.closest('li');
			// Предполагаем, что вы хотите добавить несколько изображений в карусель
			var images      = [
					'img/portfolio/1.jpg',
					'img/portfolio/2.jpg',
					'img/portfolio/3.jpg',
			...
			var carouselHtml = '<div class="owl-carousel owl-theme">';
			images.forEach(function(img){
					carouselHtml += '<div class="item"><img src="' + img + '" alt=""></div>';
			});
			carouselHtml += '</div>';

			modalBox.addClass('opened');
			modalBox.find('.description_wrap').html(details);
			// Заменяем статическое изображение на карусель
			modalBox.find('.popup_details').prepend(carouselHtml);
			modalBox.find('.popup_details .owl-carousel').after('<div class="portfolio_main_title"><span class="category">'+category+'</span><h3 class="title">'+title+'</h3></div>');

			// Инициализация Owl Carousel
			modalBox.find('.owl-carousel').owlCarousel({
				loop: false,
				items: 1,
				autoplay: false,
				dots: false,
				nav: true,
				navText: [
						'<svg width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.17374 0.177653L4.53352 0.537184C4.65179 0.661774 4.71094 0.807288 4.71094 0.973392C4.71094 1.14292 4.65179 1.28671 4.53352 1.40498L2.20241 3.7352L4.53345 6.0654C4.65172 6.18366 4.71087 6.32748 4.71087 6.49694C4.71087 6.66312 4.65172 6.80861 4.53345 6.93321L4.17368 7.28802C4.05223 7.40951 3.9067 7.47021 3.73721 7.47021C3.56457 7.47021 3.42068 7.40944 3.30559 7.28804L0.183176 4.16675C0.0617164 4.05164 0.000937306 3.90787 0.000937299 3.73522C0.000937291 3.5658 0.0616997 3.42034 0.183176 3.29897L3.30559 0.177671C3.42392 0.0593949 3.56779 0.000215104 3.73721 0.000215096C3.90354 0.0001984 4.04899 0.0593777 4.17374 0.177653Z" fill=""></path></svg>', // Путь к вашей SVG кнопке "назад"
						'<svg width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg"><pathd="M0.536858 0.177166L0.177307 0.5366C0.0591135 0.661157 -2.91168e-07 0.806632 -2.83909e-07 0.972691C-2.76501e-07 1.14217 0.0591135 1.28592 0.177307 1.40417L2.50697 3.73376L0.177375 6.06333C0.0591808 6.18156 6.71968e-05 6.32534 6.72042e-05 6.49476C6.72115e-05 6.6609 0.0591808 6.80634 0.177375 6.93092L0.536925 7.28563C0.658292 7.40708 0.803734 7.46777 0.973118 7.46777C1.14566 7.46777 1.28945 7.40701 1.40447 7.28565L4.52494 4.1652C4.64632 4.05011 4.70707 3.90638 4.70707 3.73377C4.70707 3.56441 4.64634 3.41898 4.52494 3.29765L1.40447 0.177183C1.28621 0.0589389 1.14243 -0.00022464 0.973117 -0.000224633C0.80689 -0.000241315 0.661532 0.0589223 0.536858 0.177166Z"fill="" /></svg>'
					]
...
function elisc_tm_modalbox_portfolio(){
	...
	button.on('click',function(){
			...
			var imagesCount = parseInt(element.data('images-count'), 10);
			var images      = [];

			// Генерация путей к изображениям на основе названия проекта и количества изображений
			for (let i = 1; i <= imagesCount; i++) {
					images.push(`img/portfolio/${projectName}/${i}.png`);
			}

			var details     = element.closest('li').find('.hidden_content_portfolio').html();
			var category    = element.closest('li').find('.details .category').html();
			var title       = element.closest('li').find('.details .title a').text();
			...
			modalBox.find('.owl-carousel').owlCarousel({
					...
					navText: [
						'<svg width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.17374 0.177653L4.53352 0.537184C4.65179 0.661774 4.71094 0.807288 4.71094 0.973392C4.71094 1.14292 4.65179 1.28671 4.53352 1.40498L2.20241 3.7352L4.53345 6.0654C4.65172 6.18366 4.71087 6.32748 4.71087 6.49694C4.71087 6.66312 4.65172 6.80861 4.53345 6.93321L4.17368 7.28802C4.05223 7.40951 3.9067 7.47021 3.73721 7.47021C3.56457 7.47021 3.42068 7.40944 3.30559 7.28804L0.183176 4.16675C0.0617164 4.05164 0.000937306 3.90787 0.000937299 3.73522C0.000937291 3.5658 0.0616997 3.42034 0.183176 3.29897L3.30559 0.177671C3.42392 0.0593949 3.56779 0.000215104 3.73721 0.000215096C3.90354 0.0001984 4.04899 0.0593777 4.17374 0.177653Z" fill=""></path></svg>', // Путь к вашей SVG кнопке "назад"
						'<svg width="5" height="8" viewBox="0 0 5 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.536858 0.177166L0.177307 0.5366C0.0591135 0.661157 -2.91168e-07 0.806632 -2.83909e-07 0.972691C-2.76501e-07 1.14217 0.0591135 1.28592 0.177307 1.40417L2.50697 3.73376L0.177375 6.06333C0.0591808 6.18156 6.71968e-05 6.32534 6.72042e-05 6.49476C6.72115e-05 6.6609 0.0591808 6.80634 0.177375 6.93092L0.536925 7.28563C0.658292 7.40708 0.803734 7.46777 0.973118 7.46777C1.14566 7.46777 1.28945 7.40701 1.40447 7.28565L4.52494 4.1652C4.64632 4.05011 4.70707 3.90638 4.70707 3.73377C4.70707 3.56441 4.64634 3.41898 4.52494 3.29765L1.40447 0.177183C1.28621 0.0589389 1.14243 -0.00022464 0.973117 -0.000224633C0.80689 -0.000241315 0.661532 0.0589223 0.536858 0.177166Z" fill=""></path></svg>'
					]
			});
